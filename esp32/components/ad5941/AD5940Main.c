 ...
 *****************************************************************************
 @file:    AD5940Main.c
 @author:  Neo Xu (have been modified)
...
AD5940Err EDAShowResult(void *pData, uint32_t DataCount, QueueHandle_t ad5941_queue)
{
  float RtiaMag;
  /*Process data*/
  ...
  for (int i = 0; i < DataCount; i++)
  {
    ...
    xQueueSendToBack(ad5941_queue, &mag, portMAX_DELAY);
    // ESP_LOGI(TAG, "Rtia:%.2f,(Real,Image):(%.2f,%.2f)Ohm---Mag:%.2fOhm,Phase:%.2f`",
...
void AD5940_Main(void *pvParametersd)
{
  ad5941_info_t ad5941_info = *(ad5941_info_t *)pvParametersd;
  ...
  ESP_LOGI(TAG, "AD5941 initialization complete, free heap size %u.", esp_get_free_heap_size());

  while (1)
  {
    xSemaphoreTake(ad5941_info.gpio0_semaphore_handle, portMAX_DELAY);
    // TickType_t start_time = xTaskGetTickCount();
    temp = APPBUFF_SIZE;
    AppEDAISR(AppBuff, &temp);                              /* Deal with it and provide a buffer to store data we got */
    EDAShowResult(AppBuff, temp, ad5941_info.ad5941_queue); /* Show the results to UART */
